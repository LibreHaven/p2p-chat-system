...
class PeerService {
  ...
  createPeer(id, onOpen, onError, onConnection) {
    return this._createPeerWithRetry(id, onOpen, onError, onConnection);
  }

  /**
   * 带重试机制的Peer创建
   */
  _createPeerWithRetry(id, onOpen, onError, onConnection) {
   ...
   * 尝试下一个服务器
   */
  _tryNextServer(id, onOpen, onError, onConnection) {
    ...
    if (this.currentServerIndex >= config.peerServers.length) {
      ...
      setTimeout(() => {
        this.isRetrying = false;
        this._createPeerWithRetry(id, onOpen, onError, onConnection);
