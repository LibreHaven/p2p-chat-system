...
const useConnection = ({
  ...
  const handleReceivedData = useCallback((data) => {
    ...
    if (parsedData.type === 'connection-request') {
      ...
      // 更新connectionSlice中的useEncryption信息
      if (window.updateConnectionRequestEncryption) {
        window.updateConnectionRequestEncryption(parsedData.peerId, receivedEncryption);
      }
